...
9. **INCIDENCIA RECURRENTE — Ventas del PC que no llegan a la nube (12-09-2026, aún SIN solución definitiva):**
   ...
   - **ACTUALIZACIÓN 12-09 (noche, II): FASE 1 ACTIVADA Y VERIFICADA + FASE 2 IMPLEMENTADA** —
     ...
     (d) **Pendiente descubierto**: 12 `saleNumber` duplicados en el Doc 60 (#759–#763 y #809–#815, ventas distintas con mismo número) — la numeración local `max(local)+1` del PC colisiona con la nube; la FASE 2 alinea el máximo local (#817) y la FASE 3B lo elimina estructuralmente. La próxima venta del PC duplicará #816 hasta ejecutar FASE 2. Renumeración quirúrgica de los 12 pares: pendiente (procedimiento snapshot→update→verificar).
