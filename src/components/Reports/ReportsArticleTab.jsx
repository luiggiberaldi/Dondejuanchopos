...
export default function ReportsArticleTab({
    ...
    return (
        <div className="space-y-4 animate-in fade-in duration-200">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white dark:bg-surface-900 rounded-2xl p-4 border border-slate-200 dark:border-surface-800 shadow-sm">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-brand/10 dark:bg-brand/20 text-brand dark:text-brand flex items-center justify-center font-bold shrink-0">
                        <Package size={20} />
                    </div>
                    <div>
                        <h3 className="font-bold text-slate-800 dark:text-white text-base">Reporte de Ventas por Artículo</h3>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                    {selectedArticleIds.length > 0 && (
                        <button
                            ...
                            className="w-full sm:w-auto flex items-center justify-center px-3 py-2.5 min-h-[44px] text-xs font-bold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/50 rounded-xl hover:bg-rose-100 transition-all active:scale-95 shrink-0"
                        >
                            Limpiar Selección ({selectedArticleIds.length})
                        </button>
                    )}
                    ...
                    <button
                        ...
                        className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 min-h-[48px] bg-brand hover:bg-brand-dark disabled:bg-slate-300 dark:disabled:bg-surface-700 text-white font-bold rounded-xl text-xs md:text-sm shadow-primary-tone active:scale-95 transition-all"
                    >
                        <Download size={16} />
                        {isGeneratingPdf 
                            ? 'Generando PDF...' 
                            : selectedArticleIds.length > 0
                                ? `Exportar PDF (${selectedArticleIds.length} Seleccionados)`
                                : 'Exportar PDF de Artículos'}
                    </button>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-3 sm:flex sm:flex-wrap gap-1.5">
                        {RANGE_OPTIONS.map(opt => (
                            <button
                                ...
                                className={`px-2.5 py-2 min-h-[38px] rounded-xl text-[11px] sm:text-xs font-bold text-center transition-all active:scale-95 ${
                                    artRange === opt.id
