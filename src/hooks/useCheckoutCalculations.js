...
export function useCheckoutCalculations({
    ...
    const dynamicTotalUsd = useMemo(() => {
        if (isPureBsPayment && safeRate > 0) {
            return round2(divR(cartTotalBs, safeRate));
        }
        return cartTotalUsd;
    }, [isPureBsPayment, cartTotalUsd, cartTotalBs, safeRate]);

    const dynamicTotalBs = useMemo(() => {
        if (isPureBsPayment) {
            return cartTotalBs;
        }
        return round2(mulR(cartTotalUsd, safeRate));
    }, [isPureBsPayment, cartTotalUsd, cartTotalBs, safeRate]);
    ...
    const casheaAmountUsd = useMemo(() => {
        ...
        return round2(mulR(dynamicTotalUsd, (100 - casheaPercent) / 100));
    ...
    const _detectWarning = useCallback(() => {
        if (dynamicTotalUsd <= 0) return null;
        ...
        for (const m of paymentMethods) {
            ...
            if (m.currency === 'COP' && safeTasaCop > 100) {
                const expectedCop = mulR(dynamicTotalUsd, safeTasaCop);
                // If user entered a value that looks like USD in COP field (e.g., 50 instead of 200,000)
                if (val < expectedCop * 0.05 && val > 0 && val <= dynamicTotalUsd * 2) {
                    return {
                        type: 'currency_confusion',
                        title: 'Posible error de moneda',
                        lines: [
            ...
            const threshold = dynamicTotalUsd <= 10  ? { factor: 4,   minDiff: 15 }
                            : dynamicTotalUsd <= 50  ? { factor: 3,   minDiff: 30 }
                            : dynamicTotalUsd <= 200 ? { factor: 2,   minDiff: 50 }
                            :                      { factor: 1.5, minDiff: 100 };

            if (valUsd > dynamicTotalUsd * threshold.factor && diff > threshold.minDiff) {
                ...
                return {
                    ...
                    lines: [
                        `Ingresaste ${symbol}${val.toLocaleString('es-VE', { minimumFractionDigits: 2 })} para una venta de $${round2(dynamicTotalUsd)}.`,
        ...
        return null;
    }, [barValues, paymentMethods, dynamicTotalUsd, safeRate, safeTasaCop]);
